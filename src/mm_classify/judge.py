...
def judge_asset_labels(
    ...
    **응답 내용**에 대해서는 예외를 올리지 않는다 — LLM 이 이상한 답을 주거나 응답이 비어도
    ``ok=False`` 인 판정으로 돌려준다. 실패는 행을 남기지 않아 다음 배치가 자연히 재시도한다
    (spec §2 — 행 부재 = 미판정/실패).

    ⚠️ 다만 **전송 계층 예외**(연결 실패·타임아웃 등 ``complete_json`` 이 감싸지 않고 올리는 것)는
    **그대로 전파된다**. 자산 하나의 실패가 배치를 멈추지 않게 하려면 **호출부가 자산 단위로
    격리**해야 한다(파이프 배치 ``run_mm_classify`` 가 그렇게 한다 · ``mm_meta.judge`` 와 같은 규율).
