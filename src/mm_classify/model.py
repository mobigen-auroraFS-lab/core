...
# 084 F05 — 타입 어휘가 ``mm_skill`` 을 저장소로 빌려 쓸 때 썼던 **예약 skill_code**.
#
# 🔴 **v303(spec 086)에서 정본이 옮겨 갔다** — 타입 어휘는 이제 ``mm_meta_type_vocab``
#   (``vocab_code='default'``)에 있고, ``mm_skill`` 에는 이 코드의 행이 **없다**.
#   그럼 왜 상수를 남기나:
#     ① **되돌리기(v303 downgrade)가 이 행을 ``mm_skill`` 로 복원한다** — 그 상태에서 분류 배치가
#        어휘 행을 집으면 안 되므로 아래 격리 집합이 계속 필요하다(원복 가능성이 요구사항이다).
#     ② v303 을 아직 적용하지 않은 배포(구버전 DB)도 같은 상태다.
#   즉 이 상수의 역할은 "저장 위치"에서 **"집으면 안 되는 코드"** 로 좁아졌다.
...
#   그런데 어휘 행은 분류 대상이 **자산이 아니라 개체**이고, 판정도 별도 배치가 아니라 개체 추출과
#   같은 호출에서 난다(문맥이 있어야 동음이의를 가르므로). 방어가 없으면 어휘 행이 ``mm_skill`` 에
#   있는 상태(위 ①②)에서 전 자산이 "인물/장소/…"로 분류되기 시작한다 — 쓰이지도 않는 판정에 LLM
#   비용이 들고 패싯 축이 오염된다.
#   비유하면 창고를 같이 쓰는 것과 같은 기계를 쓰는 것의 차이다. v303 으로 **선반은 나눠졌지만**,
#   되돌리면 다시 한 선반이 되므로 컨베이어 가드는 남긴다.
