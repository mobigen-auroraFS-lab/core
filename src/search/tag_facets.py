"""검색 결과-스코프 **태그 패싯** 순수 함수 — 정규화 키·표시 라벨·집계.
...
풀에는 거의 다 들어오지만(전체 회수율 중위 100%) **상위 10건의 절반은 그 태그가 없는 자산**이다
(precision@10 중위 50%). 즉 검색은 데려오되 골라내지 못한다. 태그 필터는 정의상 정확도 100%이고,
``검은색``·``자연``·``스포츠`` 처럼 검색으로는 도달조차 못 하는 태그의 **유일한 경로**다.

여기 있는 것은 전부 **순수 함수**다 — DB·OpenSearch·LLM 을 부르지 않으며, 같은 입력이면 언제나
같은 출력이다(헌법 3조 결정 재현성. 정렬 동률 tie-break 까지 못 박는다). 서비스 레포는 이 함수를
호출하는 배선만 하고 판단 로직을 자체 구현하지 않는다(083 spec §레포 배치).
...
def display_label(originals: list[str]) -> str:
    """같은 정규화 키로 묶인 원문들 중 **화면에 보여줄 표기 하나**를 고른다.
    ...
    Args:
        originals: 한 정규화 키 그룹에 속한 원문 문자열들(같은 값이 여러 번 들어온다 — 그 횟수가
            빈도다). 문자열이 아닌 값·공백뿐인 값은 후보에서 제외한다.
...
def aggregate_tag_facets(rows: list[dict[str, Any]], *, top_n: int, min_count: int) -> dict[str, Any]:
    ...
    계수 규칙:
      ...
      - ``min_count`` 미만은 목록에서 감춘다. 실측상 태그의 83.8% 가 1건짜리라, 감추지 않으면
    ...
    for row in rows:
        if not isinstance(row, dict):
            continue
        tags = row.get("tags")
        # 문자열 하나가 오면 글자 단위로 순회돼 쓰레기 키가 생긴다 — 배열만 받는다.
        if not isinstance(tags, (list, tuple)):
            continue
        # 행 안에서 같은 원문이 반복돼도 표시 라벨 투표는 1회(행 계수와 같은 원칙).
        seen_keys: set[str] = set()
        for raw in dict.fromkeys(t for t in tags if isinstance(t, str)):
    ...
    label_by_key = {k: display_label(v) for k, v in originals_by_key.items()}
    eligible = [
        {"label": label_by_key[key], "count": count}
        for key, count in counts.items()
        if count >= min_count
    ]
    eligible.sort(key=lambda item: (-item["count"], item["label"]))
    return {
        "items": eligible[:top_n],
        "has_more": len(eligible) > top_n,
