...
# 🔴 그래서 집합 판정에는 **임계가 하나도 없다**: BM25 낱말이 맞았나 아닌가뿐이다.
#   - 점수 임계를 두지 않는 이유: 개체 검색 품질을 잴 골든이 **지금 없다**(092 의 82개 개체·질의셋은
#     코퍼스 전면 교체로 정답이 0건 — 자산 골든이 죽은 것과 같은 사유). 보정할 수 없는 숫자를
#     새로 들이면 근거 없이 고른 값이 그대로 운영에 남는다.
#   - min-max 정규화를 쓰지 않는 이유: 정규화는 **두 신호를 섞어 순위를 내려고** 있는 장치라
#     집합 판정에는 소비자가 없다. 덤으로 "후보 깊이를 늘리면 기존 상위 점수가 흔들린다"는
#     결합(099 T020)이 새 경로에서 **원천적으로 사라진다**.
#   - kNN(의미) 게이트를 쓰지 않는 이유: 게이트의 배경 수준(baseline)은 **후보 풀의 하위 절반 평균**
#     이라 풀 크기와 함께 움직인다. 집합을 완전하게 만들려면 풀을 키워야 하는데, 키우는 순간
#     게이트가 실측 보정과 다른 값이 된다(둘을 동시에 만족시킬 수 없다). 더 큰 이유는 설명 가능성이다 —
#     순위가 없으면 의미로 걸린 개체는 목록 아무 데나 흩어지고, 사용자가 치지 않은 낱말의 개체가
#     "좁히기" 결과에 남는다(091 D1 위양성 0 규율 위반).
...
# 한 번에 받아올 매칭 개체 수 상한. **순위 절단이 아니라 폭주 방지선**이다 — 현 노출 개체가
