...
# ── 개체 검색 OpenSearch 이전(092 · 2026-09-01) ─────────────────────────────
# 개체 검색만 PG pgvector 를 직접 조회해 037 "OpenSearch 단일 백엔드" 원칙에서 예외로 남아
# 있었다. 계기는 검색 결함이다 — 짧은 한국어 단어에서 임베딩이 **뜻이 아니라 글자**를 본다
# (`한글` → 한라산·한강 · 첫 글자 공유 노이즈가 무작위 대비 7배). nori 형태소가 이를 구조적으로
# 배제한다: `한글창제`→['한글','창제'] / `한라산`→['한라','산'] 이라 겹치지 않는다.
# 🔴 **자산 인덱스와 반드시 다른 이름**이어야 한다 — 같은 인덱스에 섞으면 매핑이 충돌하고
#    037 이후 안정된 자산 검색 경로에 위험이 간다.
ENTITY_INDEX_DEFAULT: str = "mm_entities"
# 필드 가중. 이름이 가장 무겁고, `member`(구성 자산 요약·키워드)가 재현율의 핵심이다 —
# 🔴 빼면 단어 하나 재현율이 **90% → 50%** 로 무너진다(착수 전 실측). 개체 설명문 한 문장(68자)에는
# 없는 낱말이 구성 자산 요약에는 있기 때문이다.
ENTITY_BM25_FIELDS_DEFAULT: tuple[str, ...] = (
    "name^3", "keywords^2", "description", "member",
)
# 🔴 **자산은 and 인데 개체는 or 다**(위 OS_BM25_OPERATOR_DEFAULT 와 다름). 자산은 문서가 길어
# 모든 낱말이 들어 있을 만하지만, 개체는 텍스트가 짧아 and 가 가혹하다 — 실측 C2 73.3%(and) vs
# **86.7%(or)**. `minimum_should_match`(50%·75%)도 재봤으나 or 과 결과가 같았다(텍스트가 짧아 무력).
# 대가로 무관 질의 결과가 0.2 → 1.4건이 되지만, BM25 노이즈는 "그 글자가 실제로 있어서" 걸린
# 것이라 화면에 근거를 보일 수 있다(임베딩 노이즈는 그게 안 된다).
ENTITY_BM25_OPERATOR_DEFAULT: str = "or"
# 융합 결과에서 화면에 낼 개수. 🔴 **090 의 3이 아니라 5다** — 실측에서 정답이 4~5위에 있는
# 경우가 많았다(`가수`→아이유 4위 · `영화`→기생충 5위 · `화산섬 여행`→제주도 5위). 관련 개체를
# 더 많이 데려오는 구조라 정답이 밀리는 것이고, 3으로 자르면 그 정답들을 통째로 잃는다.
#   상위 3: E1 80.0% · E2 80.0% · 무관 1.4건   ← 합격선에 딱 붙는다
#   상위 5: E1 90.0% · E2 90.0% · 무관 1.9건   ← 채택(합격선 +10p 여유)
#   상위 7: E1 95.0% · E2 93.3% · 무관 2.2건   ← 무관 결과 합격선(≤2.0건) 초과
ENTITY_SEARCH_TOP_N_DEFAULT: int = 5
...
# ── 집합 판정(mm-meta 화면)의 게이트 — 2026-09-21 재보정 ────────────────────────
# 🔴 **상대 격차를 끄고 절대 하한만 본다.** 위 0.15 는 개체가 82개이던 시절 값이고, 822개가 된
#    지금 개념 질의 통과율이 **3%** 다(실측 · 질의 102개). 상대 격차는 색인이 커지면 후보 20개
#    안이 빽빽해져 배경이 올라가고 줄어든다 — **자료가 늘수록 조용히 더 막히는** 구조다.
#    절대 하한은 코사인 값 자체를 보므로 색인 크기와 무관하다(파일 검색이 하한 0.60 으로
#    재보정 없이 도는 것과 같은 구조).
# 실측 근거(2026-09-21 · 개념 102 · 음성 217 = 난수 60 + **자료 밖 157**):
#   0.42 → 통과 92% · 차단 72% | **0.44 → 통과 81% · 차단 84%** | 0.46 → 통과 67% · 차단 91%
#   음성에 「주식 투자 방법」류 **자료 밖 질의**를 넣은 것이 핵심이다 — 난수만으로는 어떤
#   기준이든 다 막아내 변별이 되지 않는다.
# ⚠️ 위 「절대 하한은 두지 않는다」(2026-09-01)는 **개체가 82개일 때의 관찰**이다. 822개에서
#    다시 재니 개념(0.380~0.628)과 음성(0.306~0.503)이 갈린다. 겹침은 남지만 상대 격차보다 낫다.
# ⚠️ 임계는 이 질의 묶음에 맞춘 값이다 — 실사용 로그가 쌓이면 계기판으로 다시 본다
#    (``tools/run/eval_semantic_gate.py``).
# 🔴 상대 격차 상수는 **두지 않는다**. 0 으로 둔 값을 남기면 "언젠가 쓰겠지" 하고 검사가 따라붙고,
#    실제로 같은 값을 두 번 재는 코드가 생겼다(2026-09-21 · 읽는 사람이 두 단계로 오해했다).
ENTITY_SET_GATE_FLOOR_DEFAULT: float = 0.44
...
# ── 개체 **집합 판정**(099 G4 · 2026-09-17) ─────────────────────────────────
# 위의 ``ENTITY_SEARCH_TOP_N``·게이트·후보 깊이는 전부 **"가장 맞는 5개"를 고르는** 장치다.
# 099 는 개체 검색의 계약을 바꿨다 — 「순위」가 아니라 **「매칭 개체 집합」**을 돌려주고, 순서는
# DB 목록(구성 자산 수 내림차순)이 정하며 커서로 이어 읽는다(spec 099 §3-2a).
# 🔴 집합은 **두 갈래의 합집합**이다(2026-09-17 사용자 결정 — 초안의 "낱말만"을 뒤집었다)::
#     집합 = ① BM25 낱말 매칭 전부  ∪  ② (kNN 게이트 통과 시) kNN 창(20) 안 전부
#   - ② 를 되살린 이유: 낱말만 쓰면 **글자가 없는 매칭**을 통째로 잃는다(`발효`→김치 0건 ·
#     `도자기`→고려청자 0건). 089 이후 남은 검색 실패 30건이 그 어휘 불일치였고,
#     `090-entity-semantic-search` 스펙 하나가 통째로 그것을 풀려고 있었다.
#   - 🔴 ② 는 **새 임계를 만들지 않는다** — 순위 경로가 쓰는 게이트(아래 ENTITY_SEMANTIC_GATE_EPS)를
#     그대로 쓴다. 개체 검색 품질을 잴 골든이 **지금 없어서**(092 의 82개 개체·질의셋은 코퍼스 전면
#     교체로 정답이 0건) 새로 고른 숫자는 보정 근거가 없는 숫자가 된다.
#   - 절대 코사인 하한을 쓰지 않는 이유(2026-09-17 실 색인 실측): 무의미 질의 `존재하지않는낱말xyz`
#     1등이 0.442 인데 유관 질의 `불교 건축` 1등이 0.456 이라 값이 붙어 있다. 자산의
#     ``SEMANTIC_MIN_COSINE_DEFAULT``(0.60)를 개체에 쓰면 개체 전 구간(≤0.64)이 잘린다.
#   - 🔴 ② 의 창은 ``DEFAULT_CANDIDATE_SIZE``(20)에 **고정**하고 아래 ``MAX_HITS`` 를 쓰지 않는다.
#     창을 키우면 게이트 배경(하위 절반 평균)이 내려가 게이트가 **반드시 더 관대해진다**(099 T020
#     단조성 증명). eps 0.15 의 보정 전제를 지키려면 창이 그때와 같은 모양이어야 한다.
#   - ① 에는 여전히 임계·정규화·절단이 없다: "맞았나"만 본다. min-max 정규화는 **두 신호를 섞어
#     순위를 내려고** 있는 장치인데 집합 판정에는 그 소비자가 없다.
# 되돌림 경로는 그대로다 — 순위가 필요하면 ``search_entities_hybrid`` 가 종전 그대로 있다.

# 집합 판정이 보는 필드. 🔴 **색인 텍스트 필드 전량**(``entity_index.ENTITY_TEXT_FIELDS``)과 같아야
# 한다 — 갈리면 "색인엔 있는데 안 걸리는" 개체가 생긴다. 가중(``name^3``)을 떼는 이유는 두 가지다:
# 순위가 없어 가중이 할 일이 없고, ``match`` 절의 필드 이름 자리에 ``name^3`` 을 쓰면 그런 이름의
# 필드를 찾는다. ``member``(구성 자산 요약·키워드)를 빼면 재현율이 90%→50% 로 무너진다(092 실측).
ENTITY_MATCH_FIELDS_DEFAULT: tuple[str, ...] = (
    "name", "keywords", "description", "member",
)
# ① 낱말 갈래가 한 번에 받아올 매칭 개체 수 상한. **순위 절단이 아니라 폭주 방지선**이다 — 현 노출 개체가
# 822개(2026-09-17 실측)라 10,000 은 그 12배이고, 실질적으로 절단이 없다. 상한에 닿으면
# 집합이 불완전해지므로 ``match_entity_keys`` 가 경고 로그를 남긴다(조용히 잘리지 않게).
# ⚠️ 이 값을 **10,000 위로 올리면 안 된다** — OpenSearch 기본 ``index.max_result_window`` 가 10,000 이라
#    한 번에 더 받으려면 색인 설정을 바꾸거나 ``search_after`` 로 이어 받아야 한다(개체가 그만큼 늘면
#    그때 구조를 바꾼다. 지금 미리 만들면 쓰이지 않는 경로가 낡는다).
ENTITY_MATCH_MAX_HITS_DEFAULT: int = 10_000
