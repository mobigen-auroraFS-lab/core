...
def fetch_meta_type_vocab(conn: Connection[Any]) -> tuple[EntityTypeDef, ...]:
    ...
    왜 DB 인가: 타입이 이름뿐이면 경계를 LLM 상식이 정해 같은 개체가 자산마다 다른 타입으로 갈렸다
    (실측 7건). 정의문을 붙이자 흔들림 6종이 전부 통일됐고(2026-08-25 파일럿), 그 문구를 **코드 배포
    없이** 고칠 수 있어야 어휘를 다듬어 갈 수 있다.
...
def fetch_official_name_index(conn: Connection[Any]) -> dict[tuple[str, str], str]:
    ...
    실제 묶음을 배신했다 — dev 실측에서 ``(장소,'제주')`` 의 공식형이 자산 **1건**짜리
    '제주특별자치도' 로 정해졌고 알찬 묶음은 **13건**짜리 '제주도' 였다(강원도 2 vs 강원특별자치도 1
    도 같은 상태). 그대로 두면 앞으로 '제주' 판정이 1건짜리로 흘러가 묶음이 둘로 갈라진다. 그래서
