...
제주도 묶음이 6건에 머물고 ②동음이의('파리' 곤충/도시)를 원리적으로 가를 수 없다. 자산별로 요약
문맥을 함께 주면 둘 다 해소된다(제주도 14건·파리 곤충 제외 성공).
...
(조선=사건/조직 · 국립중앙박물관=장소/조직 등 7건), 정의문을 붙이자 흔들림 6종이 전부 통일됐다
(18자산 재판정). 정의문의 정본은 **등록 행**이며(``persist.fetch_meta_type_vocab``), 이 모듈은 받은
...
#    영영 갱신되지 않는다. 실제 재판정(dev 1,385건·LLM)은 사람이 실행한다.
