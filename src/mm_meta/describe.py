...
**길이 상한이 두 단계**인 이유(둘을 합치면 하나를 잃는다):
    ...
    - ``DESCRIPTION_TARGET_CHARS`` = 60 — **문안 규칙**(soft). LLM 에게 요구하는 길이다. 파일럿
      실측 평균 38자·최대 67자로, 60자를 조금 넘긴 1건도 문장으로는 정상이었다.
    - ``DESCRIPTION_MAX_CHARS`` — **코드 상한**(hard). 폭주(재료를 통째로 되풀거나 구성원을 모두
      나열)만 막는다. 60자에서 자르면 정상 산출물(67자)을 버리게 되고, 상한이 아예 없으면 카드
      한 줄이 무너진다. 초과는 **자르지 않고 실패**다 — 중간에서 끊긴 문장을 저장하면 그것이
      정상인지 폭주인지 나중에 알 수 없다(다음 배치가 다시 만드는 편이 낫다).

파일럿 실측(2026-08-24 · 116 묶음 전량): 실패 0 · 결정성 95.7%(111/116) · 평균 38자 · 최대 67자 ·
60자 초과 1건 · 상투어 잔존 0건 · 22초. 생성문이라 2회 완전 일치는 아니지만 **1회 생성 후 영속**
이므로 저장된 설명의 재현은 100%다(헌법 3조를 판정과 같은 방식으로 충족한다).
...
설계 배경: `specs/084-entity-bundle` spec §8-1(메타 설명 계약·파일럿 결과) · tasks T015
"""
...
# 구성원 요약을 프롬프트에 싣는 길이 상한(spec §8-1 "각 150자")의 **기본값**. 파일럿이 이 값으로
# 측정됐다 — 바꾸면 실측 수치(평균 38자·상투어 0)의 기준선이 흔들리므로 문안 판을 함께 올린다.
...
# 코드가 받아 주는 최대 길이(hard). 규칙(60자)의 **2배**로 둔다 —
#   ① 파일럿 최대 67자(초과 1건)는 정상 산출물이라 반드시 통과해야 한다(60 에서 자르면 알짜를 버린다).
#   ② 폭주는 이 값과 자릿수가 다르다: 구성원 요약 150자 × N 을 되풀거나 목록을 나열하면 수백 자가
#      된다. 2배(120자)면 정상 산출물의 여유분이면서 폭주는 확실히 걸린다.
...
class MetaDescription:
    ...
    def exceeds_target(self) -> bool:
        """문안 규칙(60자)을 넘겼는가 — 성공이지만 배치 리포트가 세어 두는 값.

        파일럿 지표 "60자 초과 N건"이 문안 품질의 신호였다(1/116). 실패는 설명이 없는 상태이므로
...
def _resolve_labels(name: str | None, entity_type: str | None) -> tuple[str, str]:
    ...
    Args:
        ...
        entity_type: 개체 타입(닫힌 5종). 빈 값이면 예외. **어휘 검사는 하지 않는다** — 저장 계층
            (``persist.ensure_entity_node``)이 이미 5종만 허용하므로 조회로 읽어 온 값은 어휘 안이고,
...
def build_description_prompt(
    ...
    *,
    ...
    문안은 **파일럿 기준선**이다(실패 0·상투어 0·평균 38자의 근거) — 고치면 실측 수치와 비교할 수
    없게 되므로 ``DESC_PROMPT_VERSION`` 을 함께 올린다. 규칙 세 줄이 각자 막는 것:
    ...
    Args:
        ...
        summary_max_chars: 구성원 요약 하나를 실을 길이 상한(글자). 기본 150 = 파일럿 기준선이며
